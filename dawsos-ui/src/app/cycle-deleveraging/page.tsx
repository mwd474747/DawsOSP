...
export default function CycleDeleveragingPage() {
  const { data, isLoading, error } = useCycleDeleveraging('main-portfolio');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600">Loading cycle deleveraging data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardHeader>
            <CardTitle className="text-red-600">Error Loading Cycle Deleveraging</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-slate-600">
              {error.message || 'Failed to load cycle deleveraging data. Please try again.'}
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              <CardTitle>Deleveraging Analysis</CardTitle>
              <CardDescription>
                Real-time deleveraging data from backend API
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {data?.result ? (
                  ...
                  <p className="text-slate-500">No deleveraging data available</p>
                )}
              </div>
            </CardContent>
          </Card>
