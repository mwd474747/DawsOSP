...
export default function HoldingDeepDivePage() {
  const { data, isLoading, error } = useHoldingDeepDive('main-portfolio');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-slate-600">Loading holding deep dive data...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardHeader>
            <CardTitle className="text-red-600">Error Loading Holding Deep Dive</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-slate-600">
              {error.message || 'Failed to load holding deep dive data. Please try again.'}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Real-time holding analysis data from backend API
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {data?.result ? (
                  ...
                  <p className="text-slate-500">No deep dive data available</p>
                )}
              </div>
            </CardContent>
          </Card>
