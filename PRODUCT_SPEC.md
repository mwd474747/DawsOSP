...
### Status Legend (2025-10-26)

- ✅ Operational in the seeded MVP stack
- ⚠️ Partial (service/agent exists but persistence/UI still pending)
- 🚧 Planned (spec only)

| Capability | Status | Notes |
|------------|--------|-------|
| Executor API + Pattern Orchestrator | ✅ | `/v1/execute` + orchestrator runtime match spec |
| ScenarioService (macro shocks + DaR) | ⚠️ | Service + seeded shocks live; persistence + UI wiring pending |
| Ratings service (rubrics + FMP transform) | ⚠️ | Rubrics + transforms completed; fundamentals caching/UI still TODO |
| Optimizer service / policy_rebalance | 🚧 | `backend/app/services/optimizer.py` scaffold, not wired to UI/pattern outputs |
| PDF reports / reports.render_pdf | 🚧 | Placeholder text returned; no WeasyPrint export yet |
| Observability (OTel/Prom/Sentry) | ⚠️ | Instrumentation toggled via `ENABLE_OBSERVABILITY`; defaults to disabled |
| Rights-enforced exports | ⚠️ | Rights registry + enforcement hooks exist; blocked until reports implementation ships |

---

## 1) Architecture (Seeded MVP)
...
* **OpenTelemetry** tracing and **Prometheus** metrics are available **when `ENABLE_OBSERVABILITY=true`**:
  - API latency histograms (labelled by `pattern_id`)
  - Pack build/pre-warm durations
  - Queue depth, DLQ size
  - Alert delivery latency
  - (Defaults: instrumentation disabled in local/dev Compose until Jaeger/Prometheus endpoints configured)

* **Sentry** hooks exist but remain **opt-in**; no DSN configured by default to avoid leaking seeded credentials.
...
* ⚠️ `holding_deep_dive` — fundamentals fallback to transformed FMP snapshot when live ingest unavailable; UI notes limitation
* ✅ `portfolio_macro_overview` — regime → factor_exposures → DaR → explain
* ⚠️ `portfolio_scenario_analysis` — ScenarioService shocks/DaR available; results not yet persisted or shown in UI charts
* ⚠️ `buffett_checklist` — quality & moat scorecard (0–10) + explainer; ratings agent operational but UI still seeded
* ⚠️ `news_impact_analysis` — NewsAPI metadata-only output under dev plan restrictions
* 🚧 `export_portfolio_report` — placeholder text until WeasyPrint + rights footer implemented
* 🚧 `policy_rebalance` — optimizer.propose_trades scaffold exists; no UI/pattern output wired
* ✅ `macro_trend_monitor` — regime trend → factor trend → suggest alert presets

**Capabilities (selected)**
`ledger.positions`, `pricing.apply_pack`, `metrics.compute_twr`, `metrics.currency_attribution`, `macro.classify_regime`, `macro.scenario_apply`, `risk.compute_dar`, `fundamentals.load(fmp)`, `corporate_actions.load(polygon)`, `ratings.dividend_safety|moat_strength|resilience`, `optimizer.propose_trades` *(🚧 stub; not surfaced)*, `news.search`, `reports.render_pdf` *(🚧 placeholder output)*, `ai.explain`.
...
- Enforcement hooks exist inside the reports service, but full PDF export is still 🚧. Until then, placeholder exports block restricted providers and explain the limitation.
...
> **Status note**: ScenarioService + DaR run in-memory today. Persistence tables (`scenario_results`, `dar_history`) and UI surfacing remain open backlog items. Ratings leverage seeded rubrics and FMP transforms; live fundamentals ingest is still limited to the cached snapshot unless valid API keys are present.

---

## 8) Service Level Objectives (SLOs) & Observability

### Observability Timeline (Current state: opt-in via `ENABLE_OBSERVABILITY`)

**S1-W2 Target**:
- Minimal OTel/Prom/Sentry **available but disabled by default**. Engineers must set `ENABLE_OBSERVABILITY=true` plus Jaeger/Prom targets to collect traces/metrics locally.
- Dashboards to build when telemetry is turned on:
...
### Sprint 4 Week 7: Ratings + Optimizer (current status: ratings ✅, optimizer 🚧)
- Buffett quality (DivSafety, Moat, Resilience with 0-10 scale)
- Nightly pre-warm (00:08 for S&P 500 holdings)
- Optimizer (mean-variance with policy constraints) — **service scaffold exists; policy_rebalance pattern/UI not yet wired**
...
### Sprint 4 Week 8: Reporting + Polish (current status: reporting 🚧)
- PDF export (WeasyPrint with rights gate already enforced) — **reports service still returns placeholder text until WeasyPrint implementation lands**
...
**Acceptance**:
- [ ] PDF export includes attribution footers *(🚧 pending reports implementation)*
...
**Compliance**:
- [ ] Rights registry enforced (hooks live; export drills will pass once PDF implementation replaces placeholder output)
...
* Generate portfolio/holding PDF *(🚧 placeholder output today; WeasyPrint + rights footers tracked in P1 backlog)*.
...
| Report Export     | `export_portfolio_report`         | PDF bytes *(placeholder string today)*                 | Rights gate enforced once WeasyPrint lands         |
