...
- **✅ Stable & Working:** 18 services (core business logic)
- **⚠️ Needs Refactoring:** 6 services (complex dependencies, duplication)
- **❌ Unused/Mock:** 4 services (dead code or mock implementations)
- **🔗 Complex Dependencies:** 5 dependency chains that create coupling
...
## 📋 Service Inventory (28 Total)

### Core Business Logic Services (18 - STABLE ✅)

#### 1. **PricingService** ✅ STABLE
...
**Recent Fix:**
- ✅ Security lookup now queries `securities` table instead of creating orphan records

---

#### 3. **RatingsService** ✅ STABLE
...
**Issue:** ⚠️ `rating_rubrics` table empty - uses hardcoded fallback  
**Fix Required:** Seed default rubrics (part of Phase 2 of simplification plan)

---

#### 4. **CurrencyAttributor** ✅ STABLE (BUT UNUSED TABLE)
...
**Complexity:** Medium  
**Refactoring Need:** ⚠️ **Table exists but unused** - service computes from `lots` directly

**Key Features:**
- Currency identity: `r_base = r_local + r_fx + (r_local × r_fx)`
- Per-currency breakdown
- Verification (within 1bp tolerance)

**Architectural Issue:**
- ⚠️ `currency_attribution` table exists (hypertable, 1 row) but service computes on-demand
...
#### 6. **FREDTransformationService** ✅ STABLE
...
#### 7. **IndicatorConfigManager** ✅ STABLE
...
#### 8. **CyclesService** ✅ STABLE (COMPLEX DEPENDENCIES)
...
#### 10. **MacroAwareScenarioService** ✅ STABLE
...
#### 11. **RiskService** ✅ STABLE (COMPLEX DEPENDENCIES)
...
**Refactoring Need:** ⚠️ **Table exists but unused** - computes factor exposures on-demand
...
**Architectural Issue:**
- ⚠️ `factor_exposures` table exists (hypertable, 1 row) but service computes on-demand
- **Pattern:** Compute-first with optional storage
- **Decision Needed:** Remove table or implement caching? (Part of simplification plan)

---

#### 12. **OptimizerService** ✅ STABLE (EXTERNAL DEPENDENCY)
**File:** `backend/app/services/optimizer.py`  
**Purpose:** Portfolio optimization using Riskfolio-Lib  
**Status:** ✅ Production-ready, requires external library  
...
**TODOs:**
- ⚠️ Line 671: Get real IP from request context
- ⚠️ Line 672: Get real user agent

---

#### 14. **AuthService** ✅ STABLE
...
#### 16. **NotificationsService** ✅ STABLE
...
#### 17. **AlertService** ✅ STABLE (COMPLEX DEPENDENCIES)
...
#### 18. **AlertDeliveryService** ✅ STABLE
...
### Infrastructure Services (4 - STABLE ✅)

#### 19. **RightsRegistry** ✅ STABLE
...
### Needs Refactoring Services (5 - ⚠️)

#### 23. **PerformanceCalculator** ⚠️ DUPLICATE IMPLEMENTATION
**File:** `backend/app/services/metrics.py`  
**Purpose:** Calculate TWR, MWR, Sharpe, Max Drawdown  
**Status:** ⚠️ **DUPLICATE** - Multiple implementations exist  
**Dependencies:** Database connection  
**Usage:** ⚠️ **NOT USED** - Agents query `portfolio_metrics` table instead  
**Complexity:** Medium  
**Refactoring Need:** 🔴 **HIGH PRIORITY** - Consolidate with `MetricsComputer`

**Issue:**
- ❌ **Duplicate Implementation:** `PerformanceCalculator` in `services/metrics.py`
- ❌ **Different Implementation:** `MetricsComputer` in `jobs/metrics.py`
- ❌ **Another Implementation:** `PerformanceSeeder` in `backend/jobs/seeds/performance_seeder.py`
- ❌ **Simple Script:** `populate_portfolio_metrics_simple.py`
...
#### 24. **RiskMetricsService** ⚠️ UNUSED CLASS
**File:** `backend/app/services/risk_metrics.py`  
**Purpose:** Risk metrics calculations (VaR, CVaR, tracking error)  
**Status:** ⚠️ **CLASS EXISTS BUT UNUSED** - `RiskMetrics` class not imported anywhere  
**Dependencies:** Database connection  
**Usage:** ❌ **NOT USED** - "risk_metrics" references are data structure keys, not service calls  
**Complexity:** Medium  
**Refactoring Need:** 🔴 **REMOVE** - Unused class, `RiskService` handles DaR (different metric)

**Issue:**
- ❌ `RiskMetrics` class exists but never imported/used
- ⚠️ "risk_metrics" appears in code but as data structure key, not service
- ⚠️ `RiskService` handles DaR (similar but different metric)
- ⚠️ May be duplicate or old implementation

**Recommendation:** ✅ **REMOVE** - Unused class, reduces confusion

---

#### 25. **FactorAnalysisService** ⚠️ PARTIALLY IMPLEMENTED
**File:** `backend/app/services/factor_analysis.py`  
**Purpose:** Factor exposure calculations via regression  
**Status:** ⚠️ **PARTIALLY IMPLEMENTED** - Used but falls back due to incomplete implementation  
**Dependencies:** Database connection, sklearn  
**Usage:** Used by `FinancialAnalyst` agent (but with fallback)  
**Complexity:** Medium  
**Refactoring Need:** ⚠️ **COMPLETE OR REMOVE** - Currently incomplete, causes fallback warnings

**Issue:**
- ⚠️ `FactorAnalysisService` (`FactorAnalyzer` class) exists but not fully implemented
- ⚠️ `FinancialAnalyst` agent imports it but uses fallback: "FactorAnalysisService not fully implemented"
- ⚠️ `RiskService` computes factor exposures differently (monte carlo vs regression)
- ⚠️ May be complementary (regression-based) vs `RiskService` (monte carlo-based)

**Code Reference:**
```python
# backend/app/agents/financial_analyst.py:807
logger.warning("Using fallback factor exposures - FactorAnalysisService not available")
```

**Recommendation:** ⚠️ **COMPLETE OR REMOVE** - Either finish implementation or remove and use `RiskService` only

---

#### 26. **FundamentalsTransformer** ✅ USED
**File:** `backend/app/services/fundamentals_transformer.py`  
**Purpose:** Transform fundamentals data (FMP) for ratings  
**Status:** ✅ **USED** - Actively used by `DataHarvester` and `RatingsAgent`  
**Dependencies:** None (standalone utility functions)  
**Usage:** Used by `DataHarvester` agent and `RatingsAgent`  
**Complexity:** Low  
**Refactoring Need:** ✅ **KEEP** - Actively used, well-integrated

**Usage Verified:**
- ✅ Imported by `DataHarvester` agent (`backend/app/agents/data_harvester.py:30`)
- ✅ Imported by `RatingsAgent` (`backend/app/agents/ratings_agent.py:34`)
- ✅ Function: `transform_fmp_to_ratings_format()` - transforms FMP data for ratings calculations

**Recommendation:** ✅ **KEEP** - Actively used, no refactoring needed

---

#### 27. **MacroDataAgent** ⚠️ SPECIALIZED DATA FETCHER
**File:** `backend/app/services/macro_data_agent.py`  
**Purpose:** Fetch non-FRED macro indicators (World Bank, IMF, Empire metrics)  
**Status:** ⚠️ **SPECIALIZED** - Focuses on non-FRED data sources  
**Dependencies:** httpx (HTTP client), World Bank API, IMF API  
**Usage:** ⚠️ **VERIFY** - Needs to check if used by `MacroHound` agent or patterns  
**Complexity:** Medium  
**Refactoring Need:** ⚠️ **VERIFY** - May be complement to `MacroService` (FRED data) or unused

**Issue:**
- ⚠️ Naming suggests it's an "agent" (should be in agents/, not services/)
- ⚠️ Focuses on non-FRED data (World Bank, IMF) vs `MacroService` (FRED)
- ⚠️ May be complementary or unused
- ⚠️ Specialized for Empire and Internal cycle metrics

**Recommendation:** ⚠️ **VERIFY USAGE** - Check if used by `MacroHound` agent. If unused, remove. If used, consider renaming to `MacroDataFetcher` (not an agent)

---

### Mock/Unused Services (4 - ❌)

#### 28. **CorporateActionsService** ❌ MOCK DATA ONLY
**File:** `backend/app/services/corporate_actions.py`  
**Purpose:** Corporate actions (dividends, splits, earnings)  
**Status:** ❌ **MOCK DATA ONLY** - Not properly implemented  
...
**Refactoring Need:** 🔴 **HIGH PRIORITY** - Implement properly or remove

**Issue:**
- ❌ Returns hardcoded mock data (AAPL, GOOGL, MSFT, T)
...
#### Chain 2: Pricing → Currency Attribution
```
PricingService
  ↓ (prices, FX rates)
CurrencyAttributor
...
**Complexity:** Medium  
**Coupling:** Low-Medium  
**Refactoring Need:** ⚠️ Table exists but unused (part of simplification)

---

#### Chain 3: Ratings → Optimizer → Scenarios
```
RatingsService
  ↓ (quality filtering)
OptimizerService
...
#### Chain 4: Alerts → Macro/Scenarios → Notifications
```
AlertService
  ↓ (evaluates)
...
#### Chain 5: Trade Execution → Audit → Reports
```
...
#### 3. Verify & Remove Duplicate/Unused Services
**Services Status (Verified):**
- ✅ `RiskMetricsService` - ❌ **UNUSED** - Remove `RiskMetrics` class
- ⚠️ `FactorAnalysisService` - ⚠️ **PARTIALLY IMPLEMENTED** - Complete or remove
- ✅ `FundamentalsTransformer` - ✅ **USED** - Keep (actively used)
- ⚠️ `MacroDataAgent` - ⚠️ **VERIFY** - Check if used, rename if kept

**Recommendation:**
1. ✅ **Remove `RiskMetrics` class** (unused, 1 hour)
2. ⚠️ **Complete or remove `FactorAnalysisService`** (decision needed, 2-4 hours)
3. ⚠️ **Verify `MacroDataAgent` usage** (30 min)
4. ✅ **Keep `FundamentalsTransformer`** (confirmed used)

**Time Estimate:** 3-5 hours (verification + removal)  
...
## 📊 Service Layer Health Summary

### ✅ Working Well (22 services)
- Core business logic services (18)
- Infrastructure services (4)

### ⚠️ Needs Refactoring (5 services)
- Duplicate implementations (1)
- Unused/duplicate services (3)
- Mock implementations (1)

### 🔗 Complex Dependencies (5 chains)
...
## 🎯 Integration with Simplification Plan
...
**Overall Service Layer Health:** ✅ **GOOD** (82% stable, 18% needs refactoring)

**Strengths:**
- ✅ Clear separation of concerns
- ✅ Well-designed dependency chains
- ✅ Production-ready core services
- ✅ Good documentation

**Weaknesses:**
- ⚠️ Duplicate implementations (metrics)
- ⚠️ Unused tables creating confusion
- ⚠️ Mock implementations (corporate actions)
- ⚠️ Some services need verification

**Recommendation:** ✅ **Proceed with Simplification Plan** - Service layer will benefit from database simplification and cleanup of unused complexity.
