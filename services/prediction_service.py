...
try:
    ...
    print("Warning: psycopg2 not installed. Prediction tracking will use in-memory storage.")
...
class PredictionService:
    ...
    def __init__(self, use_real_data: bool = True):
        ...
        self.db_available = PSYCOPG2_AVAILABLE
    ...
    def store_prediction(
        ...
        # Database is available, proceed with SQL insert
        conn = self._get_connection()
        cur = conn.cursor()
    ...
    def update_prediction_outcome(
        ...
        # Database is available, proceed with SQL update
        conn = self._get_connection()
        cur = conn.cursor()
    ...
    def get_predictions(
        ...
        if not self.db_available:
            ...
            if prediction_type:
                result = [p for p in result if p.get('prediction_type') == prediction_type]
            if status:
                result = [p for p in result if p.get('status') == status]
            return result[:limit]

        # Database is available, proceed with SQL query
        conn = self._get_connection()
    ...
    def calculate_prediction_accuracy(self, agent: Optional[str] = None) -> Dict[str, float]:
        ...
        # Check if database is available
        if not self.db_available:
            # Calculate from in-memory predictions
            evaluated = [p for p in self.predictions if p.get('status') == 'evaluated']
            if agent:
                evaluated = [p for p in evaluated if p.get('agent') == agent]
            ...
            for p in evaluated:
                by_type[p.get('prediction_type', 'unknown')].append(p.get('accuracy_score', 0))
